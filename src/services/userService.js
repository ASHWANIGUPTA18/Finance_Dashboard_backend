...
function register({ email, password, name, role = 'viewer' }) {
  ...
  const user = db.prepare('SELECT id, email, name, role, status, created_at FROM users WHERE id = ?').get(result.lastInsertRowid);
...
function login({ email, password }) {
  ...
  if (!bcrypt.compareSync(password, user.password)) throw badRequest('Invalid email or password');
...
function listUsers({ page = 1, limit = 20, status, role }) {
  ...
  const users = db.prepare(
    `SELECT id, email, name, role, status, created_at, updated_at FROM users ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`
...
function getUserById(id) {
  const user = db.prepare('SELECT id, email, name, role, status, created_at, updated_at FROM users WHERE id = ?').get(id);
